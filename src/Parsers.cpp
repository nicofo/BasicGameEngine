...
bool Parsers::parseJSONLevel(std::string filename,
	...
	if (!json.HasMember("waypoints")) { std::cerr << "JSON file is incomplete! Needs entry: waypoints" << std::endl; return false; }
	...
	//waypoints
	for (rapidjson::SizeType i = 0; i < json["waypoints"].Size(); i++) {
		//std::string light_name = json["lights"][i]["name"].GetString();
		auto json_wp = json["waypoints"][i]["position"].GetArray();
		auto json_wi = json["waypoints"][i]["next"].GetArray();

		Waypoint wp;
		wp.position = lm::vec3(json_wp[0].GetFloat(), json_wp[1].GetFloat(), json_wp[2].GetFloat());
		for (rapidjson::SizeType j = 0; j < json_wi.Size(); j++) {
			wp.next_index.push_back(json_wi[j].GetInt());
		}
		ECS.waypoints.push_back(wp);
	}
