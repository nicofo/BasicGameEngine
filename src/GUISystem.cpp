...
void GUISystem::updateTextures() {
	//for all images
	auto& text_elements = ECS.getAllComponents<GUIText>();
	for (auto& el : text_elements) {
		//check if texture has been created
		if (el.texture != 0) {
			updateTextTexture(el.texture, el.text, el.font_face, el.font_size, el.width, el.height);
		}
...
GLuint GUISystem::updateTextTexture(GLuint texture_id, std::string text, std::string font_path, int font_size, int tex_width, int tex_height) {
