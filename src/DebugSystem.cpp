...
void DebugSystem::updateimGUI_(float dt) {
	...
	if (show_imGUI_)
	{
		...
		for (auto& trans : transforms) {
			auto& entity = ECS.entities[trans.owner];
			if (ImGui::TreeNode(entity.name.c_str())) {
				//create temporary arrays with position and direction data
				float entity_pos_array[3] = { trans.position().x, trans.position().y, trans.position().z };

				//create imGUI components that allow us to change the values when click-dragging
				ImGui::DragFloat3("Position", entity_pos_array);

				//use values of temporary arrays to set real values (in case user changes)
				//trans.position = lm::vec3(entity_pos_array[0], entity_pos_array[1], entity_pos_array[2]);
				trans.position(lm::vec3(entity_pos_array[0], entity_pos_array[1], entity_pos_array[2]));
				ImGui::TreePop();
			}
		}
...
void DebugSystem::setPickingRay(int mouse_x, int mouse_y, int screen_width, int screen_height) {
	...
	float NDC_x = ((float) mouse_x / (float)screen_width) * 2.0f - 1.0f;
	float NDC_y = ((float)(screen_height - mouse_y) / (float)screen_height) * 2.0f - 1.0f;
	lm::mat4 invView;
	invView.set(cam.view_projection);
	invView.inverse();
	lm::vec4 resPoint = invView * lm::vec4(NDC_x, NDC_y, -1, 1);
	lm::vec4 normailzedPointWorld = resPoint.normalize();
	pick_ray_transform.position(cam.position);
	pick_ray_collider.direction = (lm::vec3(normailzedPointWorld.x, normailzedPointWorld.y, normailzedPointWorld.z) - cam.position).normalize();
	pick_ray_collider.max_distance = 100000.0f;
